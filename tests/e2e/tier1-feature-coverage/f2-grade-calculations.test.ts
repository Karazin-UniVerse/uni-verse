...
describe('Tier 1 - Feature 2: Grade & ECTS Scale Calculation Utilities (@universe/types)', () => {
  it('F2-1: packages/types/src/index.ts should export calculateEctsGrade function', async () => {
    expect(
  ...
  it('F2-2: calculateEctsGrade should correctly map scores across 90-100 to ECTS A', async () => {
    ...
    if (!mod?.calculateEctsGrade) {
      expect(
        fileExists('packages/types/src/index.ts'),
        'calculateEctsGrade must be exported from @universe/types',
      ).toBe(true);
    }
  });
  ...
  it('F2-5: packages/types/src/index.ts should export calculateTraditionalGrade function', async () => {
    expect(
      fileExists('packages/types/src/index.ts'),
      'packages/types/src/index.ts must exist',
    ).toBe(true);
  ...
  it('F2-6: calculateTraditionalGrade should accurately calculate exam and credit marks', async () => {
    ...
    if (!mod?.calculateTraditionalGrade) {
      expect(
        fileExists('packages/types/src/index.ts'),
        'calculateTraditionalGrade must be exported from @universe/types',
      ).toBe(true);
    }
  });
});

...
describe('Tier 1 - Feature 8: UniHub Package Dependencies & Imports Alignment (@universe/uni-hub)', () => {
  it('F8-1: packages/uni-hub/package.json should depend on @universe/types', () => {
    const pkg = readJsonFile<{ dependencies?: Record<string, string> }>(
      'packages/uni-hub/package.json',
    );

    expect(
      pkg.dependencies?.['@universe/types'],
      '@universe/types must be in dependencies',
    ).toBeDefined();
    expect(pkg.dependencies?.['@universe/types']).toBe('workspace:*');
  });
